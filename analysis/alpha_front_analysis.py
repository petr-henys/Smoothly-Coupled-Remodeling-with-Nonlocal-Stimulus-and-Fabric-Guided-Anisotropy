...
Outputs (rank 0):
    ...
    ├── alpha_front_diagnostic.png
    ├── alpha_front_timecourse.png
...
def _load_ct_density(mesh, rho_min: float, rho_max: float, comm: MPI.Comm, logger) -> Any:
    if comm.rank == 0:
...
def _plot_diagnostic(metrics: list[RunMetrics], out_dir: Path) -> None:
    apply_style()
    ...
    fig, axes = plt.subplots(1, 3, figsize=(8.0, 3.0), sharex=True)
    ...
    ax.set_xlabel(r"$\Delta\alpha_{\mathrm{front}}$ [deg]")
    ax.set_ylabel(r"RMSE in $\rho$ [g/cm$^3$]")
    ax.set_title(r"(a) CT agreement")
    ...
    ax.set_title(r"(b) Pattern agreement")
    ...
    ax.set_xlabel(r"$\Delta\alpha_{\mathrm{front}}$ [deg]")
    ax.set_ylabel(r"$\bar z_\rho - \bar z_{\mathrm{CT}}$ [mm]")
    ax.set_title(r"(c) Medial shift")

    plt.tight_layout()
    save_manuscript_figure(fig, "alpha_front_diagnostic", dpi=PUBLICATION_DPI, close=False)
    out_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_dir / "alpha_front_diagnostic.png", dpi=PUBLICATION_DPI, bbox_inches="tight", pad_inches=0.05)
    plt.close(fig)
...
def _plot_timecourses(
    ...
    if alphas:
        ...
        colors = [sm.to_rgba(float(a)) for a in alphas]

    for c, a in zip(colors, alphas):
        t, rho_bar = mean_rho_by_alpha[a]
        ax.plot(t, rho_bar, color=c, linewidth=PLOT_LINEWIDTH)

    ax.axhline(ct_mean_rho, color="black", linestyle="--", linewidth=REFERENCE_LINEWIDTH, label="CT mean")
    ax.set_xlabel("Time [day]")
    ax.set_ylabel(r"Mean density $\bar\rho$ [g/cm$^3$]")
    ax.set_title(r"Global density evolution vs $\Delta\alpha_{\mathrm{front}}$")
    ...
    plt.tight_layout()
    save_manuscript_figure(fig, "alpha_front_timecourse", dpi=PUBLICATION_DPI, close=False)
    out_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(out_dir / "alpha_front_timecourse.png", dpi=PUBLICATION_DPI, bbox_inches="tight", pad_inches=0.05)
    plt.close(fig)
...
def main() -> None:
    ...
    # CT density on mesh
    ct = _load_ct_density(mesh, rho_min=rho_min, rho_max=rho_max, comm=comm, logger=logger)

    # CT global mean density (volume-weighted)
    ...
    if comm.rank == 0 and metrics:
        ...
        _plot_diagnostic(metrics, out_dir=SWEEP_DIR)
        if mean_rho_time_by_alpha:
            _plot_timecourses(mean_rho_time_by_alpha, ct_mean_rho=ct_mean_rho, out_dir=SWEEP_DIR)
