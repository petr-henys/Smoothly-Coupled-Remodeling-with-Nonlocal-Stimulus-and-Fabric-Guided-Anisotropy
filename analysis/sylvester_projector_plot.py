...
pure NumPy setting and generates a manuscript-ready multi-panel figure:
`manuscript/images/sylvester_projectors.png`.
...
def projectors_sylvester_blended(
    X: np.ndarray,
    l1: float,
    l2: float,
    l3: float,
    *,
    ...
    """Regularized Sylvester projectors with smooth blending to isotropic/TI limits.

    Returns (P1,P2,P3) and the blending weights used.
    """
    Xs = symm(X)
    I = np.eye(3)
...
def generate_sylvester_projector_figure() -> None:
    ...
    for i, d in enumerate(deltas):
        ...
        (P1_b, P2_b, P3_b), weights = projectors_sylvester_blended(X, l1, l2, l3)
        P_plane_b = P2_b + P3_b
