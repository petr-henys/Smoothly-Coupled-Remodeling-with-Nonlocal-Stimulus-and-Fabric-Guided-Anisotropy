...
class TestMeshUnits:
    """Ensure mesh coordinates are preserved in millimetres (no scaling)."""

    def test_coordinates_preserved(self, temp_febio_file):
        """Nodes from FEBio file should be used as-is (mm)."""
        parser = FEBio2Dolfinx(str(temp_febio_file))

        bbox = parser.nodes.max(axis=0) - parser.nodes.min(axis=0)

        np.testing.assert_allclose(parser.nodes[0], [0.0, 0.0, 0.0], atol=1e-12)
        np.testing.assert_allclose(bbox, np.array([1.0, 1.0, 1.0]), atol=1e-12)
