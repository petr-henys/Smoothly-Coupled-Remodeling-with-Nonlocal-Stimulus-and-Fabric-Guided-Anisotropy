...
\item We have now added a dedicated compact sensitivity analysis for $\kappa_S$ and $\delta_0$ on the accelerated femur benchmark. Specifically, we run a full-factorial sweep over $\delta_0\in\{0.05,0.10,0.20\}$ and $\kappa_S\in\{0.2,0.4,0.8\}$ on a shortened $T=150$\,day diagnostic horizon and report the change in CT-based metrics relative to the default pair $(0.1,0.4)$. At fixed $\kappa_S=0.4$, varying $\delta_0$ changes the CT-RMSE by only $0.0019$\,g/cm$^3$, the Pearson correlation by $0.0029$, and the final mean density by $0.0033$\,g/cm$^3$. At fixed $\delta_0=0.1$, varying $\kappa_S$ changes the same quantities by $0.0128$\,g/cm$^3$, $0.0219$, and $0.0212$\,g/cm$^3$, respectively. We therefore moderate the interpretation accordingly: in the explored range, $\kappa_S$ is the more influential local calibration lever, while the default pair does not lie on a sharp optimum or instability boundary.
...
\item After the stimulus quantitative analysis: added paragraph acknowledging $D_S$ as a phenomenological parameter, discussing the mesh-based prescription of $\ell_S$, and cross-referencing the new compact $\delta_0$--$\kappa_S$ sensitivity study (highlighted in blue).
\item Added new section ``Sensitivity to mechanostat smoothing parameters'' with a compact full-factorial femur sweep and new Figure~\verb|\ref{fig:mechanostat_sensitivity}| (highlighted in blue).
\end{itemize}
...
\change
\begin{itemize}[nosep]
\item ``Reproducibility'' section: added software versions paragraph (DOLFINx 0.10.0, PETSc 3.24.1, Python 3.12.0, Basix 0.10.0, mpi4py 4.1.1; highlighted in blue).
\item Strong-scaling figure caption: added hardware specifications (AMD Ryzen 9 7950X, 16 cores / 32 threads, 128\,GB DDR5, Ubuntu 22.04; highlighted in blue).
\end{itemize}
