...
\label{sec:fp-aa}
We describe the nonlinear solver used to obtain $(\mathbf{L}^n, S^n,\rho^n)$ at a fixed time level $t^n$ from the Backward--Euler subproblems \eqref{eq:be-mech}--\eqref{eq:be-A}. Let the remodeling state be
\begin{equation*}
