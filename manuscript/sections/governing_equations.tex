...
\]
where $\widetilde\rho$ is the smooth lower clamp \eqref{eq:smooth-max-nd} and $w(\widetilde{\rho})$ is the transition function \eqref{eq:variable-exponent}. To suppress spurious response to very small deviations we apply a smooth lazy-zone gate with half-width $\delta_0\ge 0$ (dimensionless):
\[
\delta_{\mathrm{eff}} \;:=\;
\begin{cases}
