...
To isolate coupling behavior, we disable adaptive time stepping and fix $\Delta t=25~\mathrm{d}$ and $T=500~\mathrm{d}$ (20 steps).
...
The primary convergence indicator is the coupling residual $r_{\mathrm{Pic}}$ (reported as \texttt{proj\_res} in the logs), defined by \eqref{eq:fp-picard-res} and evaluated on the coupled state $(\mathbf{L},S,\rho)$.
For diagnostics we also use the per-iteration contraction estimate $\rho_k:=r_k/r_{k-1}$ and the condition number of the regularized Gram matrix (see \S\ref{sec:fp-aa}).
Performance is reported as the cumulative PETSc KSP solve time and KSP iterations aggregated over the four blocks (mechanics, fabric, stimulus, density); see \texttt{steps.csv} and \texttt{subiterations.csv}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llr}
    ...
    $\rho_{\mathrm{off}}$ & Anderson deactivation threshold & 0.3 \\
    $\rho_{\mathrm{on}}$ & Anderson activation threshold & 0.7 \\
    $P_{\rho}$ & Anderson switch patience & 4 \\
    \bottomrule
  \end{tabular}
\end{table}
  ...
  (a) Final coupling residual $r_{\mathrm{Pic}}$ per accepted step (log scale; values below $10^{-14}$ are floored for visibility), with the dashed line marking $\mathrm{tol}=10^{-6}$.
  (b) Total KSP solve time over the full run, stacked by fixed-point stop reason (\texttt{converged}, \texttt{max\_subiters}, \texttt{no\_progress}).
...
In \textbf{StiffSetup}, the reduced regularization and faster kinetics substantially weaken contractivity, and pure Picard typically fails to satisfy the coupling tolerance within $K_{\max}$ subiterations (stop reasons \texttt{max\_subiters}/\texttt{no\_progress}), wasting substantial KSP work while leaving a large final residual.
...
The implementation augments classical Anderson (Pulay/DIIS) mixing with safeguards that are essential in large-scale, strongly coupled runs:
...
\begin{figure}[htbp]
  ...
  (b) Global residual timeline with an overlaid contraction estimate $\rho_k=r_k/r_{k-1}$.
  ...
  (f) Cumulative events: stall/conditioning restarts, step limiting, Picard-mode switches, and step-level stop reasons (\texttt{max\_subiters}, \texttt{no\_progress}).}
...
\begin{figure}[htbp]
  \centering
  \IfFileExists{images/anderson_sweep.png}{%
    \includegraphics[width=1\linewidth]{images/anderson_sweep.png}%
  }{\fbox{\parbox{1\linewidth}{\centering Missing image: \texttt{images/anderson_sweep.png}}}}
  ...
  (c) Median contraction $\rho$ as a function of $(m,\beta)$ at the same $\lambda$ as in (a) (smaller is better).
