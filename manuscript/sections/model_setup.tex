...
To validate the remodeling algorithms and study fundamental adaptive responses in a controlled environment, we employ an idealized geometric model representing a bone specimen.
...
\label{eq:rho_pop}
\end{equation}
The resulting pair $\left(I_{\mathrm{pop}},\rho_{\mathrm{pop}}\right)$ represents the average femur morphology and density distribution of the HFValid cohort and is used as the reference (population) femur model in the remainder of this work. For comparisons with the bounded density model, we map $\rho_{\mathrm{pop}}$ to the FE mesh and linearly rescale it to the model interval $[\rho_{\min},\rho_{\max}]$ (see \S\ref{sec:femur-comparison}). This focuses the comparison on spatial patterns rather than absolute calibration.
