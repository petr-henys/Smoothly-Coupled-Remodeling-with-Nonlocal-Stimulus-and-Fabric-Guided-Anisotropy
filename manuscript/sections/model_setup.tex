...
\paragraph{Physiological Loading Sets.}
Similar to the femur model, time-dependent loading is approximated by discrete weighted load cases $\{c_i\}$.
The standard physiological set consists of three intensity levels representing daily activity:
\begin{itemize}
    \item \textbf{Low load:} $0.5$\,MPa (weight $w_{\mathrm{low}}=0.3$), representing rest/light activity.
    \item \textbf{Medium load:} $2.0$\,MPa (weight $w_{\mathrm{med}}=0.5$), representing normal walking.
    \item \textbf{High load:} $5.0$\,MPa (weight $w_{\mathrm{high}}=0.2$), representing impact activities.
\end{itemize}
...
Finally, the case weights $w_c$ (``day cycles'') are set to reflect relative activity frequencies, i.e., walking dominates daily cycles (order $10^3$) while stair climbing is an order of magnitude less frequent (order $10^2$), but remains mechanically more demanding \cite{Bergmann2016_StandardizedLoadsHipImplants}.
...
\toprule
Case & Activity & Why selected & $F_h$ [N] & $\alpha_{\mathrm{sag}}$ [deg] & $\alpha_{\mathrm{front}}$ [deg] & Major muscle forces & $w_c$ & Sources \\
\midrule
