...
Similar to the femur model, time-dependent loading is approximated by a finite set of load cases with weights $\{w_i\}$.
The standard physiological set consists of three intensity levels representing daily activity:
\begin{itemize}
    \item \textbf{Low load (case $i=1$):} $0.5$\,MPa (weight $w_1=0.3$), representing rest/light activity.
    \item \textbf{Medium load (case $i=2$):} $2.0$\,MPa (weight $w_2=0.5$), representing normal walking.
    \item \textbf{High load (case $i=3$):} $5.0$\,MPa (weight $w_3=0.2$), representing impact activities.
\end{itemize}
