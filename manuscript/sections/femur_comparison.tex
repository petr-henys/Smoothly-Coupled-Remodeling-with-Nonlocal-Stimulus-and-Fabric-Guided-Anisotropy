\section*{Femur prediction Analysis}
\label{sec:femur-comparison}

\paragraph{Motivation.}
The proximal femur is a stringent test of coupled mechanics--stimulus--density remodeling because the solution must simultaneously reproduce (i) a thin, high-density cortical shell and (ii) a spatially heterogeneous trabecular core under a small set of representative daily loading cases.
...
As a population reference we use the HFValid-derived template femur density (population average), represented as a volumetric CT image and mapped onto the FE mesh using inverse-distance weighting (IDW) interpolation (\texttt{intensity\_mapper.py}).
...
We compare predicted densities $\rho_r(\cdot,T)$ against the mapped CT density $\rho_{\mathrm{CT}}$ in three complementary ways:
\begin{enumerate}
  \item \textbf{Global agreement:} discrete nodal RMSE/MAE over CG1 degrees of freedom,
  \[
  ...
  \]
  together with a Pearson correlation as a scale-insensitive summary of spatial pattern agreement.
  \item \textbf{Compartment-aware error:} the same metrics restricted to CT-defined trabecular and cortical masks,
  \[
...
We already observe qualitatively that the special case $\psi_{\mathrm{ref}}^{\mathrm{trab}}=\psi_{\mathrm{ref}}^{\mathrm{cort}}$ (i.e.\ $r=1$) suppresses key spatial features in the predicted density distribution.
We therefore hypothesize that a non-unity ratio $r\neq 1$ is required to reproduce both cortical and trabecular density patterns under the same loads.
We test this hypothesis by (i) sweeping $r$ at fixed $\psi_0$, (ii) ranking runs by CT agreement metrics and contrast preservation, and (iii) visually confirming differences via distribution plots and (optionally) spatial residuum fields in ParaView.

\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=1\linewidth]{psi_ref_ratio_diagnostic.png}
  \caption{Effect of the cortical--trabecular reference-stimulus ratio $r=\psi_{\mathrm{ref}}^{\mathrm{cort}}/\psi_{\mathrm{ref}}^{\mathrm{trab}}$ on agreement with the CT-derived population template.
  Left: nodal RMSE of the final density field (total and compartment-restricted, using CT-defined cortical/trabecular masks).
  Right: predicted cortical--trabecular contrast $C(r)$ compared to the CT contrast $C_{\mathrm{CT}}$.
