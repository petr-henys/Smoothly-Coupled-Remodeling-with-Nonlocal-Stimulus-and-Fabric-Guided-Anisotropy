...
To put the direction-induced density variation in context, the spread of total RMSE across the sweep ($\approx 0.555$--$0.62$~g/cm$^3$) is comparable in magnitude to the inter-subject variability ($\pm 2\sigma$) of the mapped HFValid CT densities, which reaches 0.3--0.8~g/cm$^3$ depending on compartment and axial position.
In other words, the density changes caused by a $40^\circ$ frontal-angle sweep are of the same order as the natural morphological variability across the 101-subject cohort, suggesting that load-direction uncertainty is a relevant but not dominant source of model--data mismatch when assessed against population-level scatter.
}
