...
We probe directional sensitivity by applying a constant offset $\Delta\alpha_{\mathrm{front}}$ (in degrees) to the hip contact force frontal-plane angle $\alpha_{\mathrm{front}}$ in \emph{all} femur loading cases (heel strike, mid-stance, toe-off, stair climb), while leaving force magnitudes and muscle forces unchanged (\texttt{run\_alpha\_front\_sweep.py}).
...
All runs use the accelerated femur benchmark setup (\texttt{stiff\_params\_femur.json}) over $T=500$ days with identical numerical parameters.
...
\begin{enumerate}
  ...
  \item \textbf{Spatial sensitivity map (optional visualization).}
  To localize where density is most sensitive to the assumed hip direction, we compute a pointwise regression slope $\partial\rho/\partial\alpha_{\mathrm{front}}$ across the sweep and export it as a field for ParaView inspection (\texttt{results/alpha\_front\_sweep/alpha\_front\_sensitivity.bp}).
\end{enumerate}
...
\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=1\linewidth]{alpha_front_diagnostic.png}
  \caption{Sensitivity of simulated femur density to the hip force frontal-plane angle offset $\Delta\alpha_{\mathrm{front}}$.
  Left: distance to the population-template reference (RMSE of the final density field).
  Right: medial--lateral redistribution measured by the difference between mass-weighted medial coordinate $\bar z_\rho$ of the model and the CT template ($\bar z_\rho-\bar z_{\mathrm{CT}}$; positive indicates medial shift).
  Generated by \texttt{analysis/alpha\_front\_analysis.py}.}
  \label{fig:alpha_front_diagnostic}
\end{figure}

\begin{figure}[htbp]
  \centering
  \safeincludegraphics[width=0.75\linewidth]{alpha_front_timecourse.png}
  \caption{Domain-mean density evolution $\bar\rho(t)$ for the hip frontal-angle sweep, reconstructed from the conserved mass integral in \texttt{steps.csv}.
  The dashed line shows the CT template volume-mean density mapped to the FE mesh.
  Generated by \texttt{analysis/alpha\_front\_analysis.py}.}
  \label{fig:alpha_front_timecourse}
\end{figure}
