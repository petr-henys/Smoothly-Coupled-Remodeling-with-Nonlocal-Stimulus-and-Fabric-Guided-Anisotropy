...
\rev{Image-based finite element modelling of the proximal femur is well established \cite{Taddei2020_CTbasedFE}, and patient-specific studies further highlight the importance of subject- or lesion-specific heterogeneous material assignment \cite{Erani2019_PatientSpecificFE, Taddei2019_FemurMaterialHet}. Recent methodological reviews have also emphasized the importance of accurate CT segmentation \cite{Aldieri2024_CTSegmentation} and bilateral variability \cite{Schileo2015_BilateralVariability} when deriving population-level references.} The scans were acquired at IRCCS Istituto Ortopedico Rizzoli (Bologna) between 1999 and 2016 for surgical planning of hip arthroplasty at the contralateral femur. Besides the volumetric images, the dataset provides (i) surface segmentations and (ii) landmark coordinates (including the femoral head centre and distal landmarks), which we use for defining local reference systems and for quality control.
...
\rev{This calibration-and-mapping step should be viewed within the broader workflow of CT-derived biomarker extraction for hip-fracture risk assessment, where segmentation robustness and density calibration condition downstream FE estimates \cite{Aldieri2024_CTSegmentation}.}
...
\caption{Discrete loading cases used in the FE simulations. Hip resultant forces are parameterised in the femur coordinate system (FCS) by their magnitude $F_h$ and direction angles $(\alpha_{\mathrm{sag}},\alpha_{\mathrm{front}})$ as defined in Eq.~\eqref{eq:force_from_angles}. Muscle entries list magnitude [N] and direction angles [deg] in the same convention. The weight $w_i$ denotes the relative day-cycle frequency used when aggregating multi-case results. \rev{Force magnitudes and directions are derived from the standardized average-patient data of Bergmann et al.\ \cite{Bergmann2010_RealisticLoadsHipImplants, Bergmann2016_StandardizedLoadsHipImplants} (OrthoLoad HIP98 dataset \cite{OrthoLoadDatabase}); the specific stance-phase instants (heel strike, mid-stance, toe-off) and the stair-ascent case were selected based on reported peak-force profiles and muscle recruitment patterns.}}
